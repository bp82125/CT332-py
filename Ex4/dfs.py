...
def callOperator(currentState: StateMonad, option: int) -> StateMonad:
    if option == 1:
        ...
        return currentState.bind(lambda state: state.pourWaterXZ())
    elif option == 4:
        return currentState.bind(lambda state: state.pourWaterZX())
    elif option == 5:
        return currentState.bind(lambda state: state.pourWaterYZ())
    elif option == 6:
        return currentState.bind(lambda state: state.pourWaterZY())
    else:
        return currentState
